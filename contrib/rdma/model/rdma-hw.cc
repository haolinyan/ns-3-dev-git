...
int RdmaHw::ReceiveUdp(Ptr<Packet> p, CustomHeader &ch){
	Ipv4Address src = Ipv4Address(ch.sip);
	Ipv4Address dst = Ipv4Address(ch.dip);
	...
	std::cout << Simulator::Now() << " Node "<< m_node->GetId() << " receive UDP packet from " << src << " to " << dst << " pg:" << ch.udp.pg << " seq:" << ch.udp.seq << " " << x << std::endl;
