...
TypeId RdmaDriver::GetTypeId (void)
{
	static TypeId tid = TypeId ("ns3::RdmaDriver")
		...
		.AddTraceSource ("QpComplete", "A qp completes.",
                ...
                "ns3::RdmaDriver::QpCompleteCallback")
		;
	return tid;
}
...
void RdmaDriver::Init(void){
	...
	for (uint32_t i = 0; i < m_node->GetNDevices(); i++){
		// Ptr<QbbNetDevice> dev = nullptr;
		// if (m_node->GetDevice(i)->GetObject<QbbNetDevice>()->IsQbb())
		...
		Ptr<QbbNetDevice> dev = m_node->GetDevice(i)->GetObject<QbbNetDevice>();
