...
namespace ns3 {
	...
	TypeId RdmaEgressQueue::GetTypeId (void)
	{
		static TypeId tid = TypeId ("ns3::RdmaEgressQueue")
			...
			.AddTraceSource ("RdmaEnqueue", "Enqueue a packet in the RdmaEgressQueue.",
					...
					"ns3::RdmaEgressQueue::RdmaPktSent")
	...
	RdmaEgressQueue::RdmaEgressQueue(){
		...
		m_ackQ->SetAttribute("MaxSize", StringValue("4096MB")); // queue limit is on a higher level, not here
	}
		...
		QbbNetDevice::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::QbbNetDevice")
			...
			.AddTraceSource ("QbbEnqueue", "Enqueue a packet in the QbbNetDevice.",
					...
					"ns3::QbbNetDevice::QbbNDQueueTrace")
			...
			.AddTraceSource ("RdmaQpDequeue", "A qp dequeue a packet.",
					...
					"ns3::QbbNetDevice::QbbNDQueueCallback")
	...
	QbbNetDevice::QbbNetDevice()
	{
		...
		var->SetAttribute("Max", DoubleValue(65536));
	}
		...
		if (m_node->GetNodeType() == 0){
			...
			if (p){
				...
				if (qIndex == 0){//this is a pause or cnp, send it immediately!
					m_node->GetObject<SwitchNode>()->SwitchNotifyDequeue(m_ifIndex, qIndex, p);
					p->RemovePacketTag(t);
				}else{
					m_node->GetObject<SwitchNode>()->SwitchNotifyDequeue(m_ifIndex, qIndex, p);
					p->RemovePacketTag(t);
		...
		QbbNetDevice::Receive(Ptr<Packet> packet)
	{
		...
		Ptr<Packet> p_ = packet->Copy();
		CustomHeader ch(CustomHeader::L2_Header | CustomHeader::L3_Header | CustomHeader::L4_Header);
		ch.getInt = 1; // parse INT header
		p_->RemoveHeader(ch);

		// Ipv4Address src(ch.sip);
		// Ipv4Address dst(ch.dip);
		// std::cout <<"Node: " << m_node->GetId() << " received one packet, from " << src << " to " << dst 
		// << " srcport " << ch.udp.sport << " dstport " << ch.udp.dport << std::endl;
		...
		if (ch.l3Prot == 0xFE){ // PFC
			...
			if (m_node->GetNodeType() > 0){ // switch
				...
				m_node->GetObject<SwitchNode>()->SwitchReceiveFromDevice(this, packet, ch);
			}else { // NIC
				// send to RdmaHw
	...
	void QbbNetDevice::SendPfc(uint32_t qIndex, uint32_t type){
		...
		ipv4h.SetTtl(1);
		ipv4h.SetIdentification(var->GetValue());
		p->AddHeader(ipv4h);
		AddHeader(p, 0x800);
		CustomHeader ch(CustomHeader::L2_Header | CustomHeader::L3_Header | CustomHeader::L4_Header);
		p->PeekHeader(ch);
		...
		QbbNetDevice::TransmitStart(Ptr<Packet> p)
	{
		...
		NS_LOG_UNCOND("Node " << m_node->GetId() << " dev " << m_ifIndex << " transmit a packet at " << Simulator::Now().GetNanoSeconds() << " ns txCompleteTime: " << txCompleteTime);
   ...
   void QbbNetDevice::TriggerTransmit(void){
	...
	void QbbNetDevice::UpdateNextAvail(Time t){
		if (!m_nextSend.IsExpired() && t < Simulator::GetDelayLeft(m_nextSend)){
